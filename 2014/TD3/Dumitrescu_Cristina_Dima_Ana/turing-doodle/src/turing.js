...
{
  turing.startDemo_ ();
  setTimeout (turing.startGame_, 300);
  setTimeout (turing.enterBonusMode_, 3000);
}
...
turing.init = function() {
  ...
  if (turing.isImageReady_(mainSprite)) {
    // turing.startDemo_ ();
    // turing.enterBonusMode_ ();
    enter ();
  } else {
    // mainSprite.onload = turing.startDemo_;
    // mainSprite.onload = turing.enterBonusMode_;
    mainSprite.onload = enter;
  }
};
